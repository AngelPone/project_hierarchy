...
for (batch in 0:7) {
  ...
  # natural hierarchy
  S <- read.csv("prison/S.csv")
  S <- unname(as.matrix(S))
  natural_data <- data
  
  natural_data$nl <- list(list(S = S[2:11,],
                               basef = NULL,
                               rf = NULL))
  
  natural_data <- forecast(natural_data, bfmethod, h=NROW(data$tts), frequency=4)
  ...
  for (n_repeat in c(20, 50, 100)) {
    for (n_cluster in 2:5) {
      ...
      data <- forecast(data, bfmethod, h=NROW(data$tts), frequency=4)
  ...
  data <- forecast(data, "arima", h=NROW(data$tts), frequency=4)
  data <- reconcile.all(data)
  accs <- evaluate.hts(data, metrics, type="average")
  accs_single <- evaluate.hts(data, metrics, type="nl")
